...
const SaleStatusBadge: React.FC<SaleStatusBadgeProps> = ({
  status,
  className,
}) => {
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            {customer.document && (
              <p className="text-sm text-gray-600 mt-1">{customer.document}</p>
            )}
          </div>
        </div>

        {/* Fixed: Access address fields directly from customer object */}
        {(customer.address ||
          customer.city ||
          customer.state ||
          customer.zipCode) && (
...
export default function SaleDetailPage() {
  ...
  const handleCancel = async () => {
    ...
    if (
      !confirm(
        `Tem certeza que deseja cancelar a venda ${formatSaleNumber(sale.id)}?`
      )
    ) {
      return;
    }
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      ...
      toast.success(
        `Comprovante enviado via WhatsApp para ${sale.customer.phone}`
      );
    } finally {
      setIsProcessing(null);
    }
