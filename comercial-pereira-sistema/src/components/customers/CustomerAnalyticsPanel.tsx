...
import { CustomerSegment, getCustomerSegment } from "@/types/customer";

// Define the correct interface based on what's being used in the service
...
export const CustomerAnalyticsPanel: React.FC<CustomerAnalyticsPanelProps> = ({
  ...
  const calculateRFMMetrics = (): RFMMetrics => {
    ...
    const recencyDays = lastPurchase
      ? Math.floor(
          (now.getTime() - new Date(lastPurchase).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      : 9999;
  ...
  const overallScore = Math.round(
    (rfmMetrics.recency.score +
      rfmMetrics.frequency.score +
      rfmMetrics.monetary.score) /
      3
  );
  ...
  const mockStats = {
    ...
    preferredCategories: customer.statistics.favoriteCategories.map(
  ...
  const getRecommendations = () => {
    ...
    if (rfmMetrics.recency.days > 90) {
      recommendations.push({
        ...
        description:
          "Cliente inativo há mais de 90 dias. Considere uma campanha de reativação.",
      ...
      recommendations.push({
        ...
        description:
          "Cliente não compra há um tempo. Mantenha contato regular.",
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <div
            className={`px-3 py-1 rounded-full ${overallConfig.bgColor} ${overallConfig.textColor} flex items-center gap-2`}
          >
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600 mb-3">{overallConfig.description}</p>
        ...
        <div className="mt-4 grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-blue-600">
              R$ {customer.statistics.averageOrderValue.toFixed(0)}
            </div>
            <div className="text-sm text-gray-500">Ticket Médio</div>
          </div>
          <div>
            <div className="text-lg font-bold text-purple-600">
              {customer.statistics.favoriteCategories.length}
            </div>
      ...
      {customer.statistics.favoriteCategories.length > 0 && (
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3">
            {customer.statistics.favoriteCategories
              .slice(0, 5)
              .map((category: CategoryData, index: number) => {
                const maxSpent = Math.max(
                  ...customer.statistics.favoriteCategories.map(
                    (c: CategoryData) => c.totalSpent
                  )
                );
                  ...
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-gray-500">
                        R${" "}
                        {category.totalSpent.toLocaleString("pt-BR", {
                    ...
                    <div className="text-xs text-gray-500">
                      {category.purchaseCount} compra
                      {category.purchaseCount !== 1 ? "s" : ""}
                    </div>
                  </div>
                );
              })}
