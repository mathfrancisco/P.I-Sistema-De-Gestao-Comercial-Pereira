...
export const ProductTable: React.FC<ProductTableProps> = ({
    ...
    const handleInlineEdit = (product: ProductResponse, field: string, value: string) => {
    ...
    const tableData = products.map(product => ({
        ...
        price: (
            <div
                ...
                onClick={() => handleInlineEdit(product, 'price', String(product.price))}
            >
                {editingCell?.productId === product.id && editingCell.field === 'price' ? (
                    ...
                    <span className="font-medium">R$ {new Intl.NumberFormat('pt-BR').format(product.price)}</span>
                )}
            </div>
        ),
        stock: (
            <div
                className="cursor-pointer hover:bg-gray-50 p-1 rounded"
                onClick={() => handleInlineEdit(product, 'stock', String(product.inventory?.quantity || 0))}
            >
    ...
    const tableColumns = [
        ...
        { key: 'stock', label: 'Estoque', sortable: true, align: 'center' as const },
