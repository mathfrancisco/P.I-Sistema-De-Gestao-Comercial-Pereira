...
export function useSupplierWithProducts(id: number | null) {
  const { data, isLoading, error, refetch } = useQuery<SupplierWithProducts>({
    ...
    queryFn: async () => {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Erro ao buscar fornecedor com produtos"
        );
      }
...
export function useSupplierStats() {
  const { data, isLoading, error, refetch } = useQuery<SupplierStats>({
...
export function useSupplierPerformance(id: number | null) {
  const { data, isLoading, error, refetch } = useQuery<SupplierPerformance>({
    ...
    queryFn: async () => {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Erro ao buscar performance do fornecedor"
        );
      }
...
export function useSupplierSearch(searchParams: SupplierSearchParams | null) {
  const { data, isLoading, error } = useQuery<SupplierResponse[]>({
