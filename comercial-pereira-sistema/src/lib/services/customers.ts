...
export class CustomerService {
  ...
  static async findMany(
    ...
    try {
      ...
      const formattedCustomers = customers.map((customer: any): CustomerListItem => {
  ...
  static async create(data: CreateCustomerInput): Promise<CustomerListItem> {
    try {
      ...
      if (validatedData.email) {
        ...
        if (existingEmail) {
          throw new ApiError('Email já está sendo usado por outro cliente', 409)
        }
      }
      ...
      if (validatedData.document) {
        ...
        if (existingDocument) {
          throw new ApiError('Documento já está sendo usado por outro cliente', 409)
        }
      }
  ...
  static async findByIdWithStats(customerId: number): Promise<CustomerWithStatistics> {
    try {
  ...
  static async update(
    ...
    try {
      // Validate input data
      const validatedData = updateCustomerSchema.parse({ ...data, id: customerId })
