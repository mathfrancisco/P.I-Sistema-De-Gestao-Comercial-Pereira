...
export class SaleService {
   ...
   */
  async listSales(filters: SaleFiltersInput, user: AuthUser): Promise<SalesListResponse> {
    // Validar filtros
   ...
   * Cria uma nova venda
   */
  async createSale(data: CreateSaleInput, user: AuthUser): Promise<SaleResponse> {
    // Validar dados
    ...
    const itemsWithPrices = await this.prepareItemsWithPrices(validatedData.items)
   ...
   */
  async updateSale(saleId: number, data: UpdateSaleInput, user: AuthUser): Promise<SaleResponse> {
    // Validar dados
    ...
    // Verificar permissões
    if (!canEditSale(user.role, parseInt(user.id), existingSale.userId, existingSale.status as SaleStatus)) {
   ...
   */
  async cancelSale(saleId: number, user: AuthUser, reason?: string): Promise<{ message: string; sale: any }> {
    // Verificar se venda existe
    const existingSale = await prisma.sale.findUnique({
      where: { id: saleId },
      include: {
   ...
   */
  async addSaleItem(saleId: number, data: AddSaleItemInput, user: AuthUser): Promise<any> {
    // Validar dados
    ...
    const total = calculateItemTotal(validatedData.quantity, unitPrice, validatedData.discount || 0)
   ...
   */
  async updateSaleItem(saleId: number, itemId: number, data: UpdateSaleItemInput, user: AuthUser): Promise<any> {
    // Validar dados
    ...
    if (validatedData.quantity && validatedData.quantity !== existingItem.quantity) {
    ...
    const updatedItem = await prisma.$transaction(async (tx) => {
      ...
      const total = calculateItemTotal(quantity, Number(unitPrice), Number(discount))
   ...
   */
  async removeSaleItem(saleId: number, itemId: number, user: AuthUser): Promise<{ message: string }> {
    // Verificar se venda existe e pode ser editada
    await this.validateSaleEditability(saleId, user)
   ...
   */
  async confirmSale(saleId: number, user: AuthUser): Promise<{ message: string; sale: any }> {
    ...
    if (!canTransitionStatus(sale.status as SaleStatus, SaleStatus.CONFIRMED)) {
      throw this.createError(SALE_ERROR_MESSAGES.INVALID_STATUS_TRANSITION, 409)
    }
   ...
   * Completa uma venda
   */
  async completeSale(saleId: number, user: AuthUser): Promise<{ message: string; sale: any }> {
    // Buscar venda
    const sale = await prisma.sale.findUnique({
      where: { id: saleId },
      include: {
   ...
   */
  async validateStock(data: ValidateStockInput, user: AuthUser): Promise<ValidateStockResponse> {
    // Validar dados
  ...
  private async validateSaleEditability(saleId: number, user: AuthUser) {
    ...
    if (!canEditSale(user.role, parseInt(user.id), sale.userId, sale.status as SaleStatus)) {
      throw this.createError(
  ...
  private async validateProductsAndStock(items: CreateSaleInput['items']) {
    ...
    if (stockErrors.length > 0) {
      throw this.createError(`${SALE_ERROR_MESSAGES.INSUFFICIENT_STOCK}`, 400, stockErrors)
    }
  }
  ...
  private async prepareItemsWithPrices(items: CreateSaleInput['items']) {
    ...
    return items.map(item => {
      ...
      const total = calculateItemTotal(item.quantity, unitPrice, item.discount || 0)
  ...
  private async recalculateSaleTotal(tx: any, saleId: number) {
    ...
    const total = calculateSaleTotal(subtotal, Number(sale.discount || 0), Number(sale.tax || 0))
  ...
  private createError(message: string, statusCode: number, details?: any): SaleServiceError {
