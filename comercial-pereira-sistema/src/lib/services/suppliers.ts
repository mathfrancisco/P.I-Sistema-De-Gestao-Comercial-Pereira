...
export class SupplierService {
  ...
  static async update(
    ...
    try {
      ...
      if (
        formattedData.email &&
        formattedData.email !== existingSupplier.email
      ) {
  ...
  static async search(
    searchParams: SupplierSearchInput
  ): Promise<SupplierResponse[]> {
    try {
  ...
  private static formatSupplierData(
    data: CreateSupplierInput | UpdateSupplierInput
  ) {
  ...
  private static async validateUniqueCNPJ(
    cnpj: string,
    excludeId?: number
  ): Promise<void> {
    ...
    if (!formattedCNPJ) {
      throw new ApiError("CNPJ inválido para validação", 400);
    }
  ...
  private static async logSupplierOperation(
    ...
    try {
      // Log de auditoria
      console.log(
        `[SUPPLIER_AUDIT] User ${currentUserId} performed ${action} on supplier ${targetSupplierId}`,
        {
          timestamp: new Date(),
          currentUserId,
          action,
  ...
  static formatSupplierForSelect(supplier: Supplier): {
    value: number;
    label: string;
    isActive: boolean;
  } {
