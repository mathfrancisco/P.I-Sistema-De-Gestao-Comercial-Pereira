...
export const createSaleSchema = z.object({
  customerId: z.number().int().min(1, "Cliente é obrigatório"),

  notes: z
    .string()
    .max(
      SALE_CONSTRAINTS.NOTES_MAX_LENGTH,
      `Notas devem ter no máximo ${SALE_CONSTRAINTS.NOTES_MAX_LENGTH} caracteres`
    )
    .optional()
    .nullable(),
  ...
  items: z
    .array(
      z.object({
        productId: z.number().int().min(1, "Produto é obrigatório"),
        ...
        quantity: z
          ...
          .max(
            SALE_CONSTRAINTS.MAX_QUANTITY,
            `Quantidade máxima: ${SALE_CONSTRAINTS.MAX_QUANTITY}`
          ),
        ...
        unitPrice: z
          ...
          .max(
            SALE_CONSTRAINTS.MAX_TOTAL,
            "Preço unitário não pode exceder o valor máximo"
          )
...
export const updateSaleSchema = z.object({
  customerId: z.number().int().min(1, "Cliente deve ser válido").optional(),
...
export const saleFiltersSchema = z
  .object({
    ...
    sortBy: z
      .enum(["saleDate", "total", "status", "createdAt"])
      .default("saleDate"),
...
export const validateStockSchema = z.object({
  items: z
    .array(
      z.object({
        productId: z.number().int().min(1, "ID do produto deve ser válido"),
...
export const applySaleDiscountSchema = z
  .object({
    ...
    reason: z
      .string()
      .max(
        SALE_CONSTRAINTS.REASON_MAX_LENGTH,
        `Motivo deve ter no máximo ${SALE_CONSTRAINTS.REASON_MAX_LENGTH} caracteres`
      )
      .optional(),
  })
  .refine(
    (data) => {
      if (
        ...
        data.discountValue > SALE_CONSTRAINTS.MAX_DISCOUNT_PERCENTAGE
      ) {
        return false;
      }
      return true;
...
export const saleIdSchema = z.object({
  id: z.coerce.number().int().min(1, "ID deve ser um número positivo"),
});
...
export const validateSaleBusinessRules = (
  data: CreateSaleRequest | UpdateSaleRequest
): string[] => {
  const errors: string[] = [];
